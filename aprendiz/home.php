<?php
include('../aprendiz/base.php');
?>

<div class="courses">

  <div class="my-courses course-1">
    <h2 class="h2">Curso A2</h2>
    <img class="curso-a2" src="../images/conduccion-A2.svg" alt="Curso-de-motos.svg" />
    <button class="buttom buttom-open">Ver contenido</button>
    <!-- Contenido del A2-->
    <!--Fin del contenido-->
  </div>

  <div class="my-courses course-2">
    <h2 class="h2">Curso B1</h2>
    <img class="curso-a2" src="../images/conduccion-B1.svg" alt="Curso-de-vehiculos-particulares.svg" />
    <button class="buttom buttom-open2">Ver contenido</button>

  </div>

  <div class="my-courses course-3">
    <h2 class="h2">Curso C1</h2>
    <img class="curso-a2" src="../images/conduccion-C1.svg" alt="Curso-de-vehiculos-publicos.svg" />
    <button class="buttom buttom-open2">Ver contenido</button>

    <script src="../js/index.js"></script>
  </div>

  <div class="my-courses course-4">
    <h2 class="h2">Curso Montacargas</h2>
    <img class="curso-a2" src="../images/montacargs.svg" alt="Curso-de-montacargas.svg" />
    <button class="buttom buttom-open2">Ver contenido</button>
  </div>
</div>

<!-- Contenido de los cursos-->

<div class="contentA2">
  <section class="sectionA2">
    <h1>Contenido del curso A2</h1>
    <button class="buttom buttom2">X</button>
    <section class="curse-1">
      <article class="adaptation">
        <h2>Adaptación al medio</h2>
        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ullam assumenda dolore, enim at sapiente provident suscipit fuga autem quia molestiae, nisi dolores praesentium, voluptatem rem tempora? Harum placeat repellendus error?</p>
      </article>
    </section>
  </section>
</div>

</body>

</html>
<script src="../js/index.js"></script>